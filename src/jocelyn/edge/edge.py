...
class Edge:
    ...
    def _get_path_data(self, threshold):
        image = self.image
        path = self.path
    ...
    def get_path_measurements(self, limit = 100):
        path_data, path_limits, path_widths = self._get_path_data(limit)



        for key, width in path_widths.iteritems():
            print key, width

        return path_data, path_limits, path_widths